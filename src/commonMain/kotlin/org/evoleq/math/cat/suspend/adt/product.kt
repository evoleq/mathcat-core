...
// TODO(make every function suspend)
...
interface Product<F1, F2> {
     ...
     */
        @MathCatDsl
        fun <F1, F2> fromPair(): suspend (Pair<F1, F2>)->Product<F1, F2> = {it.asProduct()}
        
        @MathCatDsl
        fun <F1, F2> toPair(): suspend (Product<F1, F2>)->Pair<F1, F2> = {it.asPair()}
    
        @MathCatDsl
        fun <F1,F2> p1(): suspend (Product<F1, F2>)->F1 = {product -> product.factor1}
    
        @MathCatDsl
        fun <F1,F2> p2(): suspend (Product<F1, F2>)->F2 = {product -> product.factor2}
    }
}
